...
def main(epochs):
    ...
    print("phantom_np.shape: ", phantom_np.shape)
    print("data_np.shape: ", data_np.shape)
    print("fbp_np.shape: ", fbp_np.shape)
    ...
    # Now, we need to cast the ODL operators as torch operators so that we can integrate them with other learnable units. For brevity, we will omit the suffix _torch in the variable and operator names.
    ...
    class LGD_net(nn.Module):
        ...
        def forward(self, y, x_init):
            ...
            for k in range(self.niter):
                grad = adj_op(fwd_op(x) - y)
                dx = self.prox[k](x, grad)
                x = x - self.step_size[k] * dx
            return x
    ...
    plt.suptitle(
        f"Comparison of FBP, TV, and LGD  (trained for {epochs} epochs) reconstructions"
    )
    plt.tight_layout()
    plt.savefig("outputs/q3b.png", dpi=300)
    print("[INFO]] Output image saved at outputs/q3b.png")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Q3b: Learned Gradient Descent for CT Reconstruction"
    )
    parser.add_argument(
        "epochs", type=int, help="Number of epochs for training the LGD network"
    )
